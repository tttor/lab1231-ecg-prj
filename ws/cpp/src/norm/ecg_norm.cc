...
void fft(double * signal_real, double *signal_imag, int length, int p, int q, vector<double> & terima_invers_bit);
void inversefft(double* real, double* imag,int n, vector<double> & terima_invers_bit);
...
char ECGNORM::norm_lead(int sample_len,vector<double> & signal_real, int *letak_r_in, int *jarak_per_bit_in, int beat_num, vector<double> & vsignal_per_lead){
   using namespace std;
    char resx = 0;
    int i,j,k,l;
   ...
   // double terima_invers_bit[150];//harusnya 128
    ...
    //for(i=0;i<128;i++){
        ...
        //}
    int letak_r[beat_num+2];  
    int jarak_per_bit[beat_num+1];
    
    for(i=0;i<beat_num;i++){
		letak_r[i]=letak_r_in[i];
		jarak_per_bit[i]=jarak_per_bit_in[i];
		}
	   
	letak_r[beat_num]=letak_r[beat_num-1]+jarak_per_bit[beat_num-1];
	letak_r[beat_num+1]=sample_len;
	jarak_per_bit[beat_num]=sample_len-letak_r[beat_num];
    ...
    for(i=0; i<beat_num+2-1; i++) {
       ...
       // printf("i nya : jarak %d %d", i,jarak_per_bit[i]);
		...
		//printf("terima invers bit size %d ",terima_invers_bit.size());	    
        for(l=0;l<128;l++){
