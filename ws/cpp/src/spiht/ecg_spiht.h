...
class ECGSPIHT {
 public:
  ...
  static void spiht_enc_helper(const Eigen::MatrixXd& wavelet_img, const uint8_t& x, const uint8_t& y, const uint64_t& n, Eigen::MatrixXd* LSP, Eigen::MatrixXd* LIP, Eigen::RowVectorXd* bit_str, uint16_t* bit_str_idx, uint64_t* bitctr, uint64_t* bits_LIS);
  ...
  static void get_pq(const uint8_t& case_num, const uint8_t& x, const uint8_t& y, uint8_t* p, uint8_t* q);
