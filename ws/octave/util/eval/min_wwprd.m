function min_wwprd(sample_num, group, order, res)
% Calculate minimum WWPRD from multiple experiments
    ...
    %proc_list = [1 9 10 11 12 15 19 22 24 39 42 43 51 55 63 65];
    proc_list = [12];
    CR = [24];
    ...
    min_wwprd_arr = zeros(size(proc_list,2),size(CR,2));
    ...
    for k = proc_list
        ...
        for l = 1:size(CR,2)
            ...
            for m = 1:repeat
                ...
                wwprd1 = wwprd_all(recg8_pre,ecg8_pre,1);
            ...
            min_wwprd_arr(row,l) = min;
        end
    end
    out_fname = [num2str(sample_num) group_fname order_fname res_fname ...
                 '-min-wwprd.txt'];
    save(out_fname,'min_wwprd_arr','-ascii','-tabs');
end
